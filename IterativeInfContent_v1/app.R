...
server <- function(input, output, session) {
  ...
  output$varplot<-renderPlot({
    ...
    varmatall<- round(CRTVarGeneral(Xdes,values$m,values$rho0,values$r,values$type),10)
    ...
    for(i in 1:nrow(Xdes)){
      for (j in 1:ncol(Xdes)){
        if(is.na(Xdes[i,j])==TRUE)  varmat_excl[i,j] <- NA
          ...
          Xdesij <- Xdes
          Xdesij[i,j] <- NA
          varmat_excl[i,j] <- round(CRTVarGeneral(Xdesij,values$m,values$rho0,values$r,values$type)/varmatall,10)
        }
      }
    }
  ...
  output$varREMplot<-renderImage({
    ...
    #color_palette <-colorRampPalette(c( "yellow", "red"))(length(table(varmat_excl)))
    pal <- colorRampPalette(brewer.pal(9, "YlOrRd"))(length(unique(melted_varmatexcl_t$value))-1)
    ...
    p<-ggplot(melted_varmatexcl_t, 
              aes(x=Period, y=Sequence, fill =factor(value))) +
      #use bquote
      #ggtitle(paste("Number of iterations = ",max(melted_varmatexcl_t$iter))) +
      geom_tile(colour = "grey50") +
      scale_y_reverse(breaks=c(1:K)) +
      scale_x_continuous(breaks=c(1:T)) +
      theme(panel.grid.minor = element_blank()) +
      # geom_text(x=0.5,y=-0.35,hjust=0,aes(label=paste0("Power:",format(round(power,4)*100,2),"%")),
      #           size=4)+ 
      geom_text(aes(Period, Sequence, label = round(value,4)), color = "black", size = 4) +
      #scale_fill_manual(values = color_palette, breaks=levels(melted_varmatexcl_t$value)[seq(90, 150, by=5)])
      scale_fill_manual(values = pal, breaks=levels(melted_varmatexcl_t$value)[seq(90, 150, by=5)])+
     #labs(title = paste("Total number of iterations = ",max(melted_varmatexcl_t$iter)),caption = "iteration: {closest_state}") +
      theme(plot.title = element_text(size = 14, face = "bold.italic"),
            plot.caption = element_text(size = 14,face = "bold",hjust = 1))
      p1<-animate(p+transition_states(states=iter,   transition_length = 1, # 3/4 of total time for transitions
                                    state_length = 100, # 1/4 of time to display actual data
                                    wrap = FALSE) # no endless loop)
                ,nframes=2*max(melted_varmatexcl_t$iter)-1, fps =10 ,#The framerate of the animation in frames/sec (doesn't change anything)
                duration = max(melted_varmatexcl_t$iter)*(1/values$acrate),#total seconds to show
                end_pause =max(melted_varmatexcl_t$iter), rewind = FALSE)  
